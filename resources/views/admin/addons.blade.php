...
@section('title', '加值服務管理')
@section('page-title', 'Add-on Services')
...
<div class="grid lg:grid-cols-3 gap-8">
    ...
    <div class="bg-white border border-border p-8 h-fit">
        <h2 class="font-serif text-lg text-ink tracking-widest mb-7">新增加值服務</h2>
        <form method="POST" action="{{ route('admin.addons.store') }}" class="space-y-5">
            @csrf
            <div>
                <label class="block text-[11px] tracking-[0.3em] uppercase text-muted mb-2">名稱 *</label>
            ...
            <div>
                <label class="block text-[11px] tracking-[0.3em] uppercase text-muted mb-2">適用對象</label>
            ...
            <table class="w-full text-sm">
                <thead class="border-b border-border">
                    <tr class="text-[11px] tracking-[0.3em] uppercase text-muted">
                        ...
                        <th class="text-left px-6 py-4">適用</th>
                ...
                <tbody class="divide-y divide-border">
                    ...
                    <tr class="hover:bg-cream/50 transition-colors">
                        <td class="px-6 py-4 text-ink tracking-wide">{{ $addon->name }}</td>
                        <td class="px-6 py-4 text-muted tracking-wide">{{ ['both'=>'全部','dog'=>'狗狗','cat'=>'貓咪'][$addon->applicable_to] ?? '—' }}</td>
                        <td class="px-6 py-4 text-ink">NT$ {{ number_format($addon->price) }}</td>
                        <td class="px-6 py-4">
                            <form method="POST" action="{{ route('admin.addons.toggle', $addon) }}">
                                @csrf @method('PATCH')
                                <button type="submit"
                        ...
                        <td class="px-6 py-4 flex items-center gap-4">
                            <button onclick="openEdit({{ $addon->id }})"
                                    class="text-xs text-accent hover:text-accent-light tracking-wide transition-colors">編輯</button>
                            <form method="POST" action="{{ route('admin.addons.destroy', $addon) }}" onsubmit="return confirm('確定刪除？')">
                                @csrf @method('DELETE')
                    ...
                    {{-- Inline Edit Row --}}
                    <tr id="edit-row-{{ $addon->id }}" class="hidden">
                        <td colspan="5" class="px-6 py-5 bg-cream/40 border-t border-dashed border-border">
                            <form method="POST" action="{{ route('admin.addons.update', $addon) }}">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
                                    ...
                                    <div>
                                        <label class="block text-[10px] tracking-[0.3em] uppercase text-muted mb-1">適用對象</label>
                                        <select name="applicable_to" class="w-full border border-border px-3 py-2 text-sm text-ink bg-white focus:outline-none focus:border-accent">
                                            <option value="both" {{ $addon->applicable_to === 'both' ? 'selected' : '' }}>全部</option>
                                            <option value="dog"  {{ $addon->applicable_to === 'dog'  ? 'selected' : '' }}>狗狗</option>
                                            <option value="cat"  {{ $addon->applicable_to === 'cat'  ? 'selected' : '' }}>貓咪</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label class="block text-[10px] tracking-[0.3em] uppercase text-muted mb-1">價格 (NT$)</label>
                                        <input type="number" name="price" value="{{ $addon->price }}" required min="0"
                                               class="w-full border border-border px-3 py-2 text-sm text-ink focus:outline-none focus:border-accent">
                                    </div>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <textarea name="description" rows="2"
                                              class="w-full border border-border px-3 py-2 text-sm text-ink focus:outline-none focus:border-accent resize-none">{{ $addon->description }}</textarea>
                                </div>
                                <div class="flex items-center gap-4">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="is_active" value="1" {{ $addon->is_active ? 'checked' : '' }} class="accent-accent">
                                    ...
                                    <button type="button" onclick="closeEdit({{ $addon->id }})"
                    ...
                    @empty
                    <tr><td colspan="5" class="px-6 py-12 text-center text-muted tracking-widest">尚無資料</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
